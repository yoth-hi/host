...
const _ = fs.readFileSync("./styles.css", "utf8");
const cssDataText = _.replace(/\n|\ {2,}/g, "");
function f(x) {
    if (x >= 0 && x < 26) {
    ...
    apply(compiler) {
        compiler.hooks.emit.tap("cssCompiler", compilation => {
            ...
            assetNames.forEach(filename => {
                ...
                assetSource = `(function(){var a=document.createElement("style");a.textContent="${cssDataText.replace(
                    '"',
                    '\\"'
